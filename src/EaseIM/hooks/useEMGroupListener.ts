...
export const useEMGroupListener = () => {
    onBeforeMount(() => {
        EChatClient.addEventHandler(LISTENNER_EVENT_NAME.groups, {
            onGroupEvent(eventData) {
                const { operation, from, name, detail, id } = eventData
    ...
    onUnmounted(() => {
        EChatClient.removeEventHandler(LISTENNER_EVENT_NAME.groups)
    })
}
