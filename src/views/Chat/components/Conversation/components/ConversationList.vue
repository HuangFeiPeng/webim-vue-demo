...
import chatbotIcon from '@/assets/images/avatar/chatbot.png'
/* route */
...
const handleConversationName = computed(() => {
    return (item) => {
        if (item.conversationType === CHAT_TYPE.SINGLE) {
            return (
                (friendList.value[item.conversationKey] &&
                    friendList.value[item.conversationKey].nickname) ||
                item.conversationInfo.name
            )
        }
        if (item.conversationType === CHAT_TYPE.GROUP) {
            if (
                joinedGroupList.value[item.conversationKey] &&
                joinedGroupList.value[item.conversationKey].groupDetail
            ) {
                return joinedGroupList.value[item.conversationKey].groupDetail
                    .name
            } else if (
                joinedGroupList.value[item.conversationKey] &&
                joinedGroupList.value[item.conversationKey].groupname
            ) {
                    ...
                    <template #reference>
                        <div class="session_list_item">
                            ...
                            <div class="item_body item_main">
                                ...
                                <div class="last_msg_body">
                                    {{
                                        item.latestMessage.type !== 'inform'
